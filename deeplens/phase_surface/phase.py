...
class Phase(DeepObj):
    ...
    def diffract(self, ray, n2=1.0):
        ...
        Two conventions exist for relating the phase polynomial to wavelength:

        **Convention A — wavelength-independent phase (used here, Zemax BINARY_2):**
            The polynomial φ(x) is a fixed phase profile in radians, not tied to
            any specific wavelength. The ray deflection is wavelength-dependent
            through the λ/(2π) prefactor. This correctly models the chromatic
            dispersion of a DOE (analogous to a grating: sin θ = mλ/d).

        **Convention B — phase at design wavelength (Zemax Hologram surfaces):**
            The polynomial gives phase at a construction wavelength λ_d. The actual
            phase at wavelength λ is scaled: φ(λ) = φ_d · λ_d/λ. The deflection
            becomes ``Δsin(θ) = m · λ_d/(2π) · dφ_d/dx``, which is wavelength-
            independent. Used for holographic and optically fabricated surfaces
            that have an explicit construction wavelength parameter.
    ...
    def sag(self, x, y, valid=None):
        """Phase surface is flat, sag is always zero."""
        return torch.zeros_like(x)

    def dfdxyz(self, x, y, valid=None):
        """Surface derivatives for a flat phase surface: df/dx=0, df/dy=0, df/dz=-1."""
        zeros = torch.zeros_like(x)
        ones = torch.ones_like(x)
        return zeros, zeros, -ones

    def max_height(self):
        """Maximum valid height for a flat phase surface."""
        return self.r
    ...
    def draw_r(self):
        """Effective drawing radius for layout plots."""
        return self.r
