...
class Ray(DeepObj):
    ...
    Stores ray origins, directions, wavelength, validity mask, energy, an
    additive per-surface bend penalty, and (in coherent mode) optical path
    length.  All tensor attributes share the same batch shape
    ``(*batch_size, num_rays)``.
    ...
    Attributes:
        ...
        bend_penalty (torch.Tensor): Accumulated soft bend penalty summed
            across refractions, shape ``(*batch, num_rays, 1)``. Each surface
            adds ``softplus(cos_gate - cos(bend))`` so the value starts at 0
            and grows with cumulative bend severity.
