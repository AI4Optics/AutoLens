...
import cv2 as cv
import matplotlib
matplotlib.use("Agg", force=False)  # headless-safe; leaves existing backend alone if already set
...
# ==================================
# Training I/O: loss curve logging
# ==================================
def record_losses(history: dict, iter_idx: int, loss_map: dict) -> None:
    """Append one row to a running ``{name: [values]}`` loss history.

    New loss names are added lazily, so callers can pass whatever the
    current regularization bundle returns without pre-declaring keys.
    """
    history.setdefault("iter", []).append(int(iter_idx))
    for name, value in loss_map.items():
        history.setdefault(name, []).append(float(value))


def save_loss_curve(history: dict, result_dir: str) -> None:
    """Persist the loss history as TSV + multi-panel log-scale PNG.

    Intended to be called at ``test_per_iter`` checkpoints, so the cost
    amortizes over the evaluation pause (lens JSON write, analysis,
    ray re-sample).  Each loss gets its own subplot with independent
    y-axis — shared scales would squash constraint losses (~1e-15)
    against the main RMS loss.  Non-positive values are masked as NaN
    so matplotlib's log axis handles zeros gracefully.
    """
    iters = history.get("iter")
    if not iters:
        return

    keys = [k for k in history if k != "iter"]

    # TSV: one column per metric, chronological rows.
    tsv_path = os.path.join(result_dir, "loss_history.tsv")
    with open(tsv_path, "w") as f:
        f.write("iter\t" + "\t".join(keys) + "\n")
        for idx, it in enumerate(iters):
            row = [str(it)] + [f"{history[k][idx]:.6e}" for k in keys]
            f.write("\t".join(row) + "\n")

    # PNG: grid of subplots, one per metric.  Up to 3 columns wide;
    # rows grow as needed.  Shared x-axis (iteration).
    n = len(keys)
    if n == 0:
        return
    ncols = min(3, n)
    ...
    for idx, k in enumerate(keys):
        ...
        vals = np.asarray(history[k], dtype=float)
