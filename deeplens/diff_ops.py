"""Custom differentiable tensor operations for the optics pipeline.

Provides:
    - interp1d: 1-D linear interpolation with autograd support.
    - grid_sample_xy: torch.nn.functional.grid_sample wrapper that accepts
      xy-coordinate grids (top-left (-1, 1), bottom-right (1, -1)).
    - diff_float: straight-through float64 -> float32 downcast (preserves
      gradients in float64).
    - diff_quantize: straight-through quantizer to N levels.

Every entity here is a pure differentiable torch op. Keep it free of
experiment plumbing, plotting, and optics-specific state — those belong
in ``utils_deeplens.py`` and the ``geolens_pkg`` subpackage respectively.
"""
