...
Functions:
    ...
    - psf_center(): Reference PSF center via pinhole projection (or chief ray for
      coherent paths). Geometric :meth:`psf` with ``recenter=True`` uses the
      traced ray bundle centroid (``forward_integral`` with ``pointc=None``).
"""
...
from ..diff_ops import diff_float
...
class GeoLensPSF:
    ...
    def psf(
        ...
        Args:
            ...
            recenter (bool, optional): For ``model='geometric'``, if True (default),
                center the PSF on the valid-ray centroid of the traced bundle; if
                False, use pinhole perspective. Coherent / Huygens paths keep their
                own recentering semantics.
    ...
    def psf_geometric(
        ...
        Args:
            ...
            recenter (bool, optional): If True, center on the traced ray bundle
                centroid; if False, use pinhole projection. Defaults to True.
        ...
        # PSF patch center for binning: bundle centroid (default) vs pinhole image.
        if recenter:
            # ``forward_integral(..., pointc=None)`` uses the valid-ray centroid
            # of the same traced bundle (after ``flip_xy``), consistent with MTF.
            if bool(torch.any(ray.is_valid > 0)):
                pointc = None
            else:
    ...
    def psf_pupil_prop(
        ...
        Args:
            ...
            recenter (bool, optional): Passed to :meth:`psf` (geometric: bundle
                centroid when ``True``; coherent/Huygens: chief-ray semantics).
                Defaults to ``True``.
