...
from ..phase_surface import Phase, Binary2Phase
...
class GeoLensIO:
    ...
    * **Code V .seq**: Code V sequential format (read-only).
        ...
        for surf_idx, surf_dict in surfs_dict.items():
            if surf_idx > 0 and surf_idx < current_surf:
                ...
                if surf_dict["TYPE"] == "STANDARD":
                    ...
                    # Binary 2 (metalens / DOE) phase surface. XDAT 3..10 map
                    # to order2..order16; older files may omit XDAT9/10.
                    ...
                    s = Binary2Phase(
                        ...
                        order14=float(surf_dict.get("XDAT9", 0.0)),
                        order16=float(surf_dict.get("XDAT10", 0.0)),
    ...
    def write_lens_zmx(self, filename="./test.zmx"):
        ...
        """
        lens_zmx_str = ""
        wvls = [float(w) for w in self.wvln_ls]
        wavl_str = "WAVL " + " ".join(f"{w:.7f}" for w in wvls)
        pwav_idx = len(wvls) // 2 + 1
        if self.float_enpd:
            enpd_str = "FLOA"
        else:
    ...
    def read_lens_seq(self, filename="./test.seq"):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if line.startswith("S "):
                ...
                if current_surface:
                    if "asph_coeff_lines" in current_surface:
                        current_surface["asph_coeffs"] = GeoLensIO._parse_aspheric_coefficients(
                            current_surface.pop("asph_coeff_lines")
                        )
                    surfaces.append(current_surface)
                    surface_index += 1
            ...
            if current_surface:
                if line.startswith("CIR"):
                    ...
                    current_surface["asph_coeff_lines"] = []
                ...
                # Collect coefficient lines (A-J) and C& continuations for batch parsing.
                elif any(
                    line.startswith(p)
                    for p in [
                        "A ", "B ", "C ", "D ", "E ",
                    ...
                    current_surface.get("asph_coeff_lines")
                    and current_surface["asph_coeff_lines"][-1].rstrip().endswith("&")
                ):
                    current_surface.setdefault("asph_coeff_lines", []).append(line)
    ...
    @staticmethod
    def _parse_aspheric_coefficients(lines: list) -> dict:
        """Parse Code V aspheric coefficient lines, handling C& multi-line continuations.

        In Code V .seq exports a coefficient key followed by '&' (e.g. 'C&')
        means the value is on the next physical line. This helper resolves those
        continuations and returns a plain dict mapping A-J to floats.

        Args:
            lines: Raw text lines from the .seq file belonging to this surface's
                   aspheric coefficient block.

        Returns:
            dict: Single-letter keys (A-J) mapped to their float values.
        """
        ...
        for line in lines:
            ...
            if pending_key is not None and parts:
                try:
                    coeffs[pending_key] = float(parts[0])
                    i = 1
                except ValueError:
                    pass
                pending_key = None
            ...
            while i < len(parts):
                token = parts[i]
                bare = token.rstrip("&")
                if len(bare) == 1 and bare in VALID_KEYS:
                    if token.endswith("&"):
                        pending_key = bare
                        i += 1
                    elif i + 1 < len(parts):
                        try:
                            coeffs[bare] = float(parts[i + 1])
                            i += 2
                        except ValueError:
                            i += 1
                    else:
    ...
    def read_lens_json(self, filename="./test.json"):
        ...
        self.d_sensor = torch.tensor(d)
        self.obj_depth = float(data.get("obj_depth", data.get("depth_default", DEPTH)))
        self.wvln_primary = float(data.get("wvln_primary", DEFAULT_WAVE))
        self.wvln_ls = [float(w) for w in data.get("wvln_ls", WAVE_RGB)]
    ...
    def write_lens_json(self, filename="./test.json"):
        ...
        data["obj_depth"] = round(float(self.obj_depth), 4)
        data["wvln_primary"] = round(float(self.wvln_primary), 4)
        data["wvln_ls"] = [round(float(w), 4) for w in self.wvln_ls]
