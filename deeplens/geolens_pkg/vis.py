...
from ..config import DEPTH, SPP_PSF
from ..ray import Ray
...
class GeoLensVis:
    ...
    def draw_layout(
        ...
        Args:
            ...
            depth (float): Object distance in mm used when sampling the
                visualization rays. Defaults to ``float('inf')``.
            zmx_format (bool): Whether to use Zemax-style layout formatting.
            multi_plot (bool): Whether to create multiple subplots per field.
            lens_title (str | None): Title for the lens plot. If ``None``, a
                title is auto-generated from focal length, F/#, FoV, image
                height, wavelengths, and RMS spot radii.
            show (bool): Whether to display the plot interactively.
            num_field (int): Number of radial field positions used to compute
                RMS spot radii shown in the auto-generated title. Only used
                when ``lens_title is None``. Defaults to 3.
            wvln_list (list[float]): Wavelengths in micrometers used for the
                auto-generated title's wavelength label and the ``analysis_spot``
                call. Defaults to ``WAVE_RGB``.
            spot_num_rays (int): Rays per field point per wavelength passed to
                ``analysis_spot`` when building the title. Defaults to ``SPP_PSF``.
            num_rays_viz (int): Number of visualization rays per field position
                drawn on the layout. Defaults to 11.
            num_views (int): Number of field angles rendered on the layout.
                Defaults to 3.
        """
        num_rays = num_rays_viz
        depth = self.obj_depth if depth is None else depth
        ...
        if lens_title is None:
            ...
            wvln_nm_str = "/".join(f"{int(round(w * 1000))}" for w in wvln_list)

            if self.aper_idx is not None:
                _, pupil_r = self.calc_entrance_pupil()
                fnum = round(eff_foclen / pupil_r / 2, 2)
                lens_title = f"FocLen{eff_foclen}mm - F/{fnum} - FoV{fov_deg} - IMGH {sensor_r}mm - {wvln_nm_str} nm"
            else:
                lens_title = f"FocLen{eff_foclen}mm - FoV{fov_deg} - IMGH {sensor_r}mm - {wvln_nm_str} nm"

            # Second line: RMS spot size (um) by FoV
            spot_results = self.analysis_spot(
                num_field=num_views,
                depth=depth,
                wvln_list=wvln_list,
                num_rays=spot_num_rays,
            )
            rms_values = [v["rms"] for v in spot_results.values()]
            rms_str = "/".join(f"{r:.2f}" for r in rms_values)
            lens_title = f"{lens_title}\nRMS Spot {rms_str} μm"

        # Draw lens layout
        colors_list = ["#CC0000", "#006600", "#0066CC"] * (num_views // 3 + 1)
        ...
        if not multi_plot:
            ...
            fig.suptitle(lens_title, fontsize=10, fontname="Nimbus Sans")
            for i, fov in enumerate(fov_ls):
                wvln_i = float(wvln_list[i % len(wvln_list)])
            ...
            wvlns_multi = wvln_list
            if len(wvlns_multi) < 3:
                wvlns_multi = wvlns_multi + [wvlns_multi[-1]] * (3 - len(wvlns_multi))
            wvlns_multi = wvlns_multi[:3]
            fig, axs = plt.subplots(1, 3, figsize=(15, 5))
            fig.suptitle(lens_title, fontsize=10, fontname="Nimbus Sans")
            for i, wvln in enumerate(wvlns_multi):
                ax = axs[i]
