...
from ..config import DEPTH, EPSILON, GEO_GRID, SPP_CALC, SPP_PSF
...
class GeoLensTolerance:
    ...
    * **Monte-Carlo analysis** – statistical sampling of random manufacturing
    ...
    def loss_rms(
        ...
        """Loss function to compute polychromatic spot error RMS.

        Args:
            num_grid (int, optional): Number of grid points. Defaults to GEO_GRID.
            depth (float, optional): Depth of the lens. Defaults to DEPTH.
            num_rays (int, optional): Rays per grid point per wavelength.
                Defaults to SPP_PSF.
            wvln_list (list[float], optional): Exactly three wavelengths in
                micrometers, ordered ``[short, middle, long]``. The middle
                wavelength is traced first so the error-adaptive weight mask
                is anchored on it. Defaults to the lens RGB wavelengths.
        ...
        # Iterate middle wavelength first so the error-adaptive weight mask
        # is anchored on the reference wavelength (same ordering as optimize()).
        wvln_order = [wvln_list[1], wvln_list[0], wvln_list[2]]
        loss_rms_ls = []
        w_mask = None
        for i, wvln in enumerate(wvln_order):
    ...
    def tolerancing_monte_carlo(self, trials=200, spp=SPP_CALC, tolerance_params=None):
        ...
        with torch.no_grad():
            for i in tqdm(range(trials)):
                ...
                perturbed_merit = multi_field_merit(lens=self)
        ...
        self.refocus()
        baseline_merit = multi_field_merit(lens=self)
