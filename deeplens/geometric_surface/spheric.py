...
class Spheric(Surface):
    ...
    def _sag(self, x, y):
        """Compute surface sag z = r**2 * c / (1 + sqrt(1 - r**2 * c**2)).

        Sampling outside the spherical cap can otherwise drive the sqrt
        radicand negative and inject NaNs into optimizer state.
        """
        # Tolerance
        if self.tolerancing:
        ...
        # Compute surface sag
        r2 = x**2 + y**2
        sag = c * r2 / (1 + torch.sqrt(torch.clamp(1 - r2 * c**2, min=EPSILON)))
        return sag
    ...
    def _dfdxy(self, x, y):
        ...
        # Compute surface sag derivatives. Clamp the radicand for the same
        # reason as in ``_sag`` so gradients stay finite past the cap edge.
        r2 = x**2 + y**2
        sf = torch.sqrt(torch.clamp(1 - r2 * c**2, min=EPSILON))
        dfdr2 = c / (2 * sf)
